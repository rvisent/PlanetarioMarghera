...
static BOOL serialLoop()
{
	...
	// il timeout è 0.1 s (vedi serPortOpen)
	...
	{
		// vedi se il messaggio è completo, altrimenti riprova
		serCount += lnBytes;
		if (serCount == 1)
		{
		...
		// il secondo carattere è la lunghezza: vedi se è valido e se abbiamo abbastanza caratteri
		...
		// il valore ritornato in serNumCh è la lunghezza effettiva -2
		if (serCount < serNumCh + 2)
			return TRUE;
	}

	// se siamo arrivati qui, il numero di byte è corretto (o sovrabbondante)
	// controlla il checksum: solo l'iniziatore è escluso
	...
	else
	{
		...
		// se era un comando bCmdReq resta TRUE e verrà ripetuto da internal_poll()
		if (!internal_poll())
			// seconda chiamata se la prima era per il controllore zero
			internal_poll();
	}
...
static void timeout()
{
	...
	if (!bPollActive)
	{
		...
		// inizia la prima trasmissione (non serve doppia chiamata, non è controllore zero)
		internal_poll();
	}
	else
	{
